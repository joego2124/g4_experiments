...
typedef struct {
    ...
    volatile uint32_t PLLSAI2CFGR;   // 0x14  PLL SAI2 configuration register
    ...
    volatile uint32_t BDCR;          // 0x8C  Backup domain control register
    volatile uint32_t CSR;           // 0x90  Clock control & status register
    uint32_t RESERVED8[1];           // 0x94  Reserved
